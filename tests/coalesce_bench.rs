...
use oxcdf::read::{read_hyperslab_async_with, Hyperslab};
use oxcdf::source::{ByteSource, MemorySource};
use bytes::Bytes;
...
async fn coalescing_reduces_requests() {
    ...
    for (label, io) in [("no coalescing ", IoConfig::NONE), ("remote preset ", IoConfig::REMOTE)] {

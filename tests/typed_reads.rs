...
fn an_f32_variable_reads_as_f32_without_a_round_trip_through_f64() {
    ...
    for (i, got) in exact.iter().enumerate() {
        let b: [u8; 4] = raw.as_bytes()[i * 4..i * 4 + 4].try_into().unwrap();
        assert_eq!(got.to_bits(), f32::from_ne_bytes(b).to_bits(), "element {i}");
    }
}
...
fn any_numeric_type_converts_to_any_other() {
    ...
    assert!(f.get_values::<f32, _>(..).is_ok(), "float to narrower float");
}
...
fn a_string_variable_read_as_a_number_reports_the_stored_type() {
    ...
    assert!(v.get_values::<f64, _>(..).unwrap_err().to_string().contains("string"));
}
...
fn a_selection_of_the_wrong_rank_or_past_the_end_is_reported() {
    ...
    assert!(v.get_values::<f64, _>([0..2]).is_err(), "rank 1 on a rank-2 variable");
    assert!(v.get_values::<f64, _>([0..99, 0..6]).is_err(), "past the end");
}

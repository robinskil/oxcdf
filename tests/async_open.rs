...
use oxcdf::{AsyncFile, NetcdfFile};
...
async fn the_two_opens_report_the_same_metadata() {
    for path in corpus() {
        ...
        let file = AsyncFile::open(source.clone()).await.unwrap();
...
async fn the_two_engines_read_the_same_bytes() {
    for path in corpus() {
        ...
        for want in sync.variables() {
            ...
            let want_values = want.read().unwrap();
            let got_values = got.read().await.unwrap();

            assert_eq!(
                got_values.as_bytes(),
                want_values.as_bytes(),
                "values of {} in {path}",
                want.path
            );
            assert_eq!(got_values.shape(), want_values.shape());

            // A variable-length value lives in a heap. Compare what it resolves
            // to, not the pointer that the raw bytes hold.
            if want_values.is_variable_length() {
                assert_eq!(
                    got_values.to_strings().ok(),
                    want_values.to_strings().ok(),
                    "strings of {} in {path}",
                    want.path
                );
            }
...
async fn chunks_match_the_synchronous_engine() {
    for path in corpus() {
        ...
        for want in sync.variables() {
            ...
            let got = file.variable(&want.path).unwrap();
            let want_chunks = want.try_chunks().unwrap();
            ...
            for chunk in got_chunks.iter().take(3) {
                assert_eq!(
                    got.read_chunk(chunk).await.unwrap().as_bytes(),
                    want.read_chunk(chunk).unwrap().as_bytes(),
                    "chunk {:?} of {} in {path}",
                    chunk.offset,
                    want.path
                );
...
async fn an_open_costs_few_round_trips() {
    for path in corpus() {
        let source = Counting::open(&path);
        AsyncFile::open_with(source.clone(), OpenOptions::remote())
            .await
            .unwrap();
...
async fn a_second_read_of_one_variable_makes_no_metadata_request() {
    ...
    let source = Counting::open(&path);
    let file = AsyncFile::open_with(source.clone(), OpenOptions::remote())
        .await
        .unwrap();
...
#[tokio::test]
async fn the_async_file_is_send_and_sync() {
    fn assert_send_sync<T: Send + Sync>() {}
    assert_send_sync::<AsyncFile>();
    assert_send_sync::<oxcdf::AsyncVariable<'_>>();
}

#[tokio::test]
async fn a_variable_dereferences_to_its_metadata() {
    let path = format!("{}/test_files/test_file.nc", env!("CARGO_MANIFEST_DIR"));
    if !std::path::Path::new(&path).exists() {
        return;
    }
    let file = AsyncFile::open(Counting::open(&path)).await.unwrap();

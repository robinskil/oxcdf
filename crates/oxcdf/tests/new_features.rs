...
fn numeric_variables_alongside_strings_still_read() {
    ...
    let values = t.get_values::<f64, _>(..).unwrap();
...
fn blosclz_decodes_literals_and_back_references() {
    ...
    assert_eq!(
        unblosclz(&stream, 8).unwrap(),
        b"ababababab".to_vec()[..8].to_vec()
    );
}
...
fn the_bit_shuffle_round_trips() {
    ...
    for typesize in [1usize, 2, 4, 8] {
        let data: Vec<u8> = (0..(typesize * 16) as u32)
...
//
// Reading a ragged array has no netCDF-style call, so those tests live in
// `oxcdf-hdf5/tests/vlen_sequences.rs`. Its metadata still surfaces here.
...
fn an_szip_dataset_is_described_but_refuses_to_read() {
    ...
    assert_eq!(
        d.chunks(file.ctx()).unwrap().unwrap().len(),
        4,
        "16x8 chunks over 64x8"
    );

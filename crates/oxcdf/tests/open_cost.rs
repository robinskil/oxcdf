...
fn open_cost() {
    ...
    let slab = oxcdf_hdf5::read::Hyperslab::all(&d.shape);
    let start = Instant::now();
    for _ in 0..N {
        std::hint::black_box(oxcdf_hdf5::read::read_hyperslab(f.ctx(), d, &slab).unwrap());
    }

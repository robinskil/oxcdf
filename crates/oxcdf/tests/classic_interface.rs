...
fn the_metadata_is_the_netcdf_shape() {
    ...
    assert_eq!(
        file.dimension_len("time"),
        file.dimension("time").map(|d| d.len)
    );
}
...
#[test]
fn a_classic_variable_reports_no_chunking() {
    let Some(path) = corpus("classic.nc") else {
        return;
    };
    let file = oxcdf::open(&path).unwrap();
    ...
    // Classic files store contiguously, so `chunking` reports none, exactly as
    // the `netcdf` crate does.
    assert_eq!(v.chunking().unwrap(), None);
    assert_eq!(v.get_values::<f64, _>(..).unwrap().len() as u64, v.len());
...
#[cfg(feature = "async")]
fn async_classic(path: &str) -> oxcdf::AsyncNetcdfFile {
...
async fn an_async_classic_selection_matches_the_synchronous_one() {
    ...
    // A classic variable reports no chunking through the async handle too.
    assert_eq!(got.chunking().unwrap(), None);
    assert_eq!(
        got.get_values::<f32, _>(..).await.unwrap().len() as u64,
        got.len()
    );
    assert!(
        got.dataset().is_none(),
        "a classic variable has no HDF5 dataset"
    );
}

...
fn the_synchronous_example_works() -> oxcdf::Result<()> {
    ...
    println!(
        "{:?}",
        temp.attribute("_FillValue").map(|a| a.value.as_f64())
    );
...
fn the_types_example_works() -> oxcdf::Result<()> {
    ...
    let err = file
        ...
        .unwrap_err();
    assert!(
        matches!(err, oxcdf::Error::TypeMismatch { .. }),
        "got {err:?}"
    );
    Ok(())
}
...
fn the_char_variable_example_works() -> oxcdf::Result<()> {
    ...
    assert_eq!(v.get_raw_values(..)?.len(), 47 * 40);
    Ok(())
}
...
#[test]
fn the_raw_values_example_works() -> oxcdf::Result<()> {
    let Some(path) = corpus("legacy_v1_objheader.h5") else {
        return Ok(());
    };
    let file = oxcdf::open(&path)?;
    ...
    let _numbers: Vec<f32> = temp.get_values::<f32, _>(..)?;
    let _bytes = temp.get_raw_values(..)?;

    let _text = file.variable("/fixed_strings").unwrap().get_strings(..)?;
    Ok(())
}
...
fn the_ndarray_example_works() -> oxcdf::Result<()> {
    ...
    let counts = file.variable("/chunked_i32").unwrap().get::<i32, _>(..)?;
    assert_eq!(counts.shape(), &[40, 6]);

    // Strings come back flat: the `netcdf` crate has no shaped string read.
    let names = file.variable("/fixed_strings").unwrap().get_strings(..)?;
...
async fn the_async_interface_matches_the_table() -> oxcdf::Result<()> {
    ...
    let _ = temp.get_values::<f64, _>(..).await?;
    let _ = temp.get_values::<f64, _>([0..2, 0..2]).await?;
    let _ = temp.get_raw_values(..).await?;
    Ok(())
}
...
async fn the_object_store_example_compiles(
    ...
    let file = AsyncNetcdfFile::open_store(store, Path::from("dac/aoml/13857_prof.nc")).await?;
...
async fn the_open_store_options_compile(
    ...
    let _ = AsyncNetcdfFile::open_store_with(
        store,
        Path::from("argo.nc"),
        OpenOptions::new()

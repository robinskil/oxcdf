...
//!
//! The walk is one large recursive parser. A second asynchronous copy costs
//! more than a thousand lines. The two copies then drift apart.
//!
//! # The method
//!
//! This module runs the same parser over a byte source that holds pages in
//! memory. The parser never waits.
//!
//! 1. Fetch a window of pages.
//! 2. Run the synchronous walk over those pages.
//! 3. A read outside the pages records what it wants. It returns
//!    [`Error::Incomplete`].
//! 4. Fetch every recorded page in one batch. Go to step 2.
//! 5. A walk that finishes returns the result.
//!
//! One round is one batch of requests. The count of rounds equals the depth of
//! the dependent reads. An asynchronous parser needs the same count. This
//! method also fetches every page of one round together.
//!
//! # Cost
//!
//! A round repeats the parse of the round before it. A parse costs
//! microseconds. A round trip to object storage costs milliseconds.
//!
//! netCDF writes its metadata near the front of the file, so the first window
//! normally covers the whole walk. Most opens take one round trip.
...
impl ReplaySource {
    fn new(size: u64, page_size: usize, pages: HashMap<u64, Bytes>) -> Self {
        Self {
            size,
...
async fn fetch(
    ...
    for &index in wanted {
        ...
        if let Some(cached) = io_cache
            .filter(|c| c.page_size() == page_size)
            .and_then(|c| c.page(index))
        {
...
mod tests {
    ...
    fn a_batch_read_records_the_missing_pages_of_every_range() {
        ...
        let err = src.read_ranges(&[(0, 8), (32, 8), (80, 8)]).unwrap_err();

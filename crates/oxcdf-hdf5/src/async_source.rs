...
//!
//! This is the fetch half of the asynchronous engine. It sits beside
//! [`crate::source::ByteSource`] and does not replace it.
//!
//! # Why a separate trait
//!
//! [`crate::source::ByteSource`] is synchronous by design. The parser then
//! needs no runtime. It runs under `rayon` or under a plain thread pool.
//!
//! An asynchronous [`crate::source::ByteSource`] would move inflate and
//! unshuffle onto runtime workers. Those steps use the processor. They would
//! block the reactor.
//!
//! # Why not the blocking adapter
//!
//! [`crate::object_store_source::ObjectStoreSource`] blocks on its futures. A
//! block is only safe off a runtime worker, so every read needs
//! `spawn_blocking`.
//!
...
/// Shows a synchronous [`crate::source::ByteSource`] as an asynchronous one.
///
/// A read completes at once and never yields. Use this only where that is
/// true: an in-memory buffer, or a local file with cached pages.
///
/// This is **not** a way to put a slow blocking source behind an asynchronous
/// interface. That stalls the runtime, which the asynchronous engine exists to
/// avoid. For a real file under load, read it on a blocking thread. Use a
/// native asynchronous source instead.
#[derive(Debug)]
...
mod tests {
    ...
    async fn the_trait_is_object_safe_and_shareable() {
        let src: Arc<dyn AsyncByteSource> = Arc::new(SyncAsAsync(MemorySource::new(vec![7; 16])));

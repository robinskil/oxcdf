...
/// How many chunks past a read to fetch and decode at once.
///
/// A scan walks chunks in order, so the next chunks are the likely next need.
/// One batched call costs one round trip. Separate calls cost one each.
...
mod tests {
    ...
    fn a_miss_decodes_and_a_hit_does_not() {
        ...
        let second = cache
            .get_or_decode(100, || panic!("must not decode again"))
            .unwrap();
...
///
/// This sits below [`ChunkCache`]. That cache holds decoded chunks. This one
/// holds the bytes any read touched.
///
/// Small reads cluster. An open walks object headers, heaps and B-trees that
/// sit near each other. A scan then reads chunks from the same region. Each
/// read is a separate request, and a remote request costs milliseconds.
///
/// # Why pages, not exact ranges
///
/// An arbitrary range needs an interval search to answer "is this covered".
/// A fixed page needs one hash lookup. Pages also merge by themselves: two
/// reads 100 bytes apart share a page, so the second is free.
///
/// The cost is up to one page of bytes nobody asked for. Any block cache makes
/// the same trade.
///
/// A hit takes no lock. Two threads that race a cold page both fetch it.
#[derive(Clone, Debug)]
pub struct IoCache {
...
impl IoCache {
    ...
    ///
    /// `page_size` is the input request size. Every miss fetches a whole page,
    /// so a larger value gives fewer and bigger requests.
    ///
    /// Use 256 KiB for object storage, where a request costs milliseconds. The
    /// 64 KiB default suits a local file, where an over-read is waste.
    ...
    ///
    /// The cache is synchronous. Only the fetch awaits. The page bookkeeping,
    /// the run detection and the assembly are shared.
    #[cfg(feature = "async")]
        ...
        for (slot, page) in pages.iter().enumerate() {
            ...
            if from < to {
                out.extend_from_slice(
                    &page[(from - page_start) as usize..(to - page_start) as usize],
                );
            }
        }
        if out.len() != len {

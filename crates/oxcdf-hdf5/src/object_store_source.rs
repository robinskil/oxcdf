...
//!
//! This reads S3, GCS, Azure and plain HTTP. It needs no local copy. The whole
//! reader addresses bytes by absolute offset and never seeks, so a file on
//! object storage is a byte range server.
//!
//! # Why this fits
//!
//! Each chunk is an independent byte range, so a hyperslab read fetches only
//! the chunks it touches. [`ByteSource::read_ranges`] sends those fetches
//! together through [`ObjectStore::get_ranges`], which also merges neighbours.
//!
//! # The blocking bridge
//!
//! `ObjectStore` is asynchronous. [`ByteSource`] is synchronous. The parser
//! stays synchronous on purpose: it then needs no runtime.
//!
//! The bridge blocks on the asynchronous call. That is only safe off a runtime
//! worker thread. Read through this source inside
//! [`tokio::task::spawn_blocking`], or on any other thread.
//!
//! A call from a runtime worker thread panics. It does not deadlock quietly.
//!
//! For an asynchronous source that needs no bridge, use
//! `async_source::AsyncObjectStoreSource`.

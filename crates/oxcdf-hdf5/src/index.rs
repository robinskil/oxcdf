...
use crate::message::{
    Attribute, Dataspace, Datatype, FillValue, FilterPipeline, Layout, LinkTarget,
};
...
///
/// ```no_run
/// # use oxcdf_hdf5::index::{Hdf5File, OpenOptions};
...
impl DatasetIndex {
    ...
    pub fn chunks(&self, ctx: Ctx<'_>) -> Result<Option<&[btree1::ChunkRecord]>> {
        ...
        Ok(self.chunks.get().expect("just set").as_deref())
    }
    ...
    fn resolve_chunks(&self, ctx: Ctx<'_>) -> Result<Option<Vec<btree1::ChunkRecord>>> {
        match &self.layout {
            Layout::Chunked {
                ...
                crate::message::ChunkIndex::BtreeV1 => match address {
                ...
                other => {
                    ...
                    Ok(Some(crate::chunk_index::read(
                        ctx,
                        other,
                        *address,
                        &self.shape,
...
fn read_attributes(ctx: Ctx<'_>, header: &ObjectHeader) -> Result<(Vec<Attribute>, bool)> {
    ...
    if let Some(info) = header.attribute_info(sizes)? {
        if let Some(heap_address) = info.fractal_heap_address {
            let (dense, all_found) = crate::dense::read_dense_attributes(
                ctx,
                heap_address,
...
fn resolve_children(ctx: Ctx<'_>, header: &ObjectHeader) -> Result<Vec<Child>> {
    ...
    if let Some(st) = header.symbol_table(sizes)? {
        ...
        for node_address in node_addresses {
            ...
            let raw = ctx.read(node_address, SymbolTableNode::encoded_len(count, sizes))?;
    ...
    if let Some(info) = header.link_info(sizes)? {
        if let Some(heap_address) = info.fractal_heap_address {
            let links = crate::dense::read_dense_links(ctx, heap_address, info.name_btree_address)?;
            for link in links {
                if let LinkTarget::Hard { address } = link.target {
                    out.push(Child {
                        name: link.name,
...
fn build_dataset(
    ...
    let dataspace = header.dataspace(sizes)?.unwrap_or(Dataspace {
        kind: crate::message::DataspaceKind::Scalar,
        dims: Vec::new(),
        max_dims: None,
    });
    let datatype = header
...
mod tests {
    ...
    fn resolves_the_chunk_index_at_open_time() {
        ...
        let chunks = d
            .chunks(file.ctx())
            .unwrap()
            .expect("a chunked dataset has an index");

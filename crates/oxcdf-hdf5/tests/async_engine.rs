...
use oxcdf_hdf5::async_source::{AsyncByteSource, SyncAsAsync};
use oxcdf_hdf5::index::Hdf5File;
use oxcdf_hdf5::read::{read_hyperslab, read_hyperslab_async, Hyperslab};
use oxcdf_hdf5::source::MemorySource;
...
async fn an_unprepared_chunked_variable_is_refused() {
    ...
    assert!(
        matches!(err, oxcdf_hdf5::Error::BadRequest(_)),
        "got {err:?}"
    );

...
use crate::hdf5::btree1::ChunkRecord;
use crate::hdf5::btree2::BtreeV2;
use crate::hdf5::context::Ctx;
use crate::hdf5::message::ChunkIndex;
...
fn read_fixed_array(ctx: Ctx<'_>, address: u64, expected: usize) -> Result<Vec<Element>> {
    ...
    let version = cur.u8()?;
    if version != 0 {
        return Err(Error::unsupported(format!("fixed array header version {version}")));
    }
...
fn read_extensible_array(ctx: Ctx<'_>, address: u64) -> Result<Vec<Element>> {
    ...
    let index_len =
        4 + 1 + 1 + o + index_block_elements * element_size + total_addresses * o + 4;
...
fn read_data_block(
    ...
    let sizes = ctx.sizes();
    let len =
        4 + 1 + 1 + sizes.offset as usize + block_offset_size + capacity * element_size + 4;

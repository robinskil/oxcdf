...
use crate::hdf5::message::filter::{id, Filter, FilterPipeline};
...
pub fn unblosclz(data: &[u8], expected_len: usize) -> Result<Vec<u8>> {
    ...
    while pos < data.len() {
        ...
        let start = out
            ...
            .ok_or_else(|| Error::malformed("blosclz match points before the start of the block"))?;
...
pub fn verify_and_strip_fletcher32(mut data: Vec<u8>) -> Result<Vec<u8>> {
    ...
    let stored_le = u32::from_le_bytes([data[split], data[split + 1], data[split + 2], data[split + 3]]);
...
mod tests {
    use super::*;
    use crate::hdf5::message::filter::FilterPipeline;
    use std::io::Write;
    ...
    fn an_unknown_filter_is_refused_even_when_marked_optional() {
        ...
        f.flags = crate::hdf5::message::filter::FLAG_OPTIONAL;
